    ...
    <el-dialog v-model="showMetaDialog" title="编辑文章元数据" width="500px">
      <el-form label-width="80px">
        ...
        <el-form-item label="标签">
          <el-select v-model="metaData.tags" multiple filterable allow-create default-first-option placeholder="选择或输入标签"
            style="width: 100%" />
...
import { postArticle, getCategoryList, getTagList } from '@/apis/articles';
...
const saveMetaData = async () => {
  ...
  // 3. 回写到 article
  article.value.categories = finalCats.map(c => c.id || c.name)
  article.value.tags       = finalTags.map(t => t.id || t.name)
...
onMounted( async () => {
  ...
  allCategories.value = cats.data.map(c => c.name)
